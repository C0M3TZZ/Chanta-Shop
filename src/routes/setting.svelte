...
<script>
	...
	let validateForm = () => {
		...
		if (
			new RegExp('^(http|https)://').test(modalForm.img) === false ||
			modalForm.img.trim().length < 1
		) {
						...
						<tr>
							...
							<td class="flex items-center justify-center">
								<label
									...
									class="btn btn-sm modal-button">EDIT</label
								>
								<label
									for="deleteModal"
									class="btn btn-sm modal-botton"
									on:click={() => {
										selectItem(product);
									}}>DEL</label
								>
	...
	<label class="modal-box relative" for="">
		...
		<div class="modal-body">
			...
			<input
				...
				class="input input-bordered w-full max-w mb-2"
			/>
			<p class="mt-2 pl-2">Description</p>
			<input
				type="text"
				placeholder="Type here"
				bind:value={modalForm.desc}
				class="input input-bordered w-full max-w mb-2"
			/>
...
<label for="deleteModal" class="modal cursor-pointer">
	<label class="modal-box relative" for="">
		...
		<div class="modal-body">
			<p class="mt-2 pl-2">
				Do you want to Delete <span class="text-error font-bold">{selectProduct.title}</span> ?
			</p>
			<div class="modal-action">
